  ...
  const handleSubmit = async (e) => {
    ...
    if (res && res.success) {
      if (res.user && res.user.role !== 'superadmin') {
        logout();
        if (res.user.role === 'operationadmin') {
          toast.error('Access denied. Operations Admin can login only on the Operations login page.');
        } else if (res.user.role === 'user') {
          toast.error('Access denied. Customer can login only on the Customer portal.');
        } else {
      ...
      <div style={styles.card} className="glass-card animate-fade-in">
        <div style={styles.header}>
          <div style={styles.iconCircle}>
            <KeyRound size={28} color="var(--highlight)" />
          </div>
          <h2 style={styles.title}>Super Admin Login</h2>
          <p style={styles.subtitle}>Enter credentials to access the super admin analytics panel</p>
        </div>
