...
import { 
  ParkingSquare, LayoutDashboard, Calendar, LogOut, User, Menu, X,
    ...
    <nav style={styles.nav}>
      <div style={styles.container}>
        ...
        <Link to="/" style={styles.logo} onClick={() => setMobileMenuOpen(false)}>
          <ParkingSquare size={28} color="var(--success)" />
          <span style={styles.logoText}>Park<span style={{ color: 'var(--success)' }}>Smart</span></span>
        </Link>
 
        {/* Desktop Center Links */}
        <div className="nav-center" style={styles.navCenter}>
          <Link to="/" style={isActive('/') ? styles.activeLink : styles.link}>Home</Link>
          <Link to="/map" style={isActive('/map') ? styles.activeLink : styles.link}>Parking Map</Link>
          {user && (
            <Link to="/bookings" style={isActive('/bookings') ? styles.activeLink : styles.link}>
              <Calendar size={16} style={{ marginRight: 4 }} /> My Bookings
            </Link>
          )}
        </div>
        ...
        <div className="nav-right" style={styles.navRight}>
          {user ? (
              ...
              {dropdownOpen && (
                <div style={styles.dropdownMenu} className="glass-card" onClick={(e) => e.stopPropagation()}>
                  <div style={styles.dropdownHeader}>
                    ...
                    <span style={styles.roleBadge}>{user.role === 'user' ? 'CUSTOMER' : user.role.toUpperCase()}</span>
                  </div>
            ...
            <div style={styles.authButtons}>
              ...
              <Link to="/register" className="btn btn-success" style={{ padding: '0.5rem 1rem' }}>Register</Link>
            </div>
          )}
        </div>
      ...
      {mobileMenuOpen && (
        <div className="mobile-menu" style={styles.mobileMenu}>
          <Link to="/" style={styles.mobileNavLink(isActive('/'))} onClick={() => setMobileMenuOpen(false)}>Home</Link>
          <Link to="/map" style={styles.mobileNavLink(isActive('/map'))} onClick={() => setMobileMenuOpen(false)}>Parking Map</Link>
          
          {user ? (
            <>
              <Link to="/bookings" style={styles.mobileNavLink(isActive('/bookings'))} onClick={() => setMobileMenuOpen(false)}>
                My Bookings
              </Link>
            ...
            <div style={styles.mobileAuthButtons}>
              <Link to="/login" className="btn btn-outline" style={{ width: '100%' }} onClick={() => setMobileMenuOpen(false)}>Login</Link>
              <Link to="/register" className="btn btn-success" style={{ width: '100%' }} onClick={() => setMobileMenuOpen(false)}>Register</Link>
            </div>
          )}
        </div>
      )}
