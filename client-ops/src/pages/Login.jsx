...
const Login = () => {
  ...
  useEffect(() => {
    if (user && user.role === 'operationadmin') {
      navigate('/dashboard');
    }
  }, [user, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (res && res.success) {
      if (res.user && res.user.role !== 'operationadmin') {
        logout();
        if (res.user.role === 'superadmin') {
          toast.error('Access denied. Super Admin can login only on the Super Admin login page.');
        } else {
          toast.error('Access denied. This portal is for operations admins only.');
        }
        return;
      }
    ...
    <div className="page-wrapper" style={styles.container}>
      <div style={styles.card} className="glass-card animate-fade-in">
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>Enter credentials to access the operations panel</p>
        </div>
